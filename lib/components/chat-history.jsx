...
export default function ChatHistory({ chats, currentChatId, setCurrentChatId, deleteChat, renameChat, }) {
    ...
    return (<div className="space-y-1 p-2">
      {chats.map((chat) => (<div key={chat.id} className={cn("dark:text-white text-black flex items-center justify-between p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer group", chat.id === currentChatId ? " bg-gray-300 dark:bg-gray-700" : "")}>
          <div className="flex items-center gap-2 flex-1 min-w-0" onClick={() => editingChatId !== chat.id && setCurrentChatId(chat.id)}>
            <div className="relative">
              ...
              {chat.lastModelUsed && (<div className={cn("absolute -bottom-1 -right-1 w-2 h-2 rounded-full", getModelColor(chat.lastModelUsed))}></div>)}
            ...
            {editingChatId === chat.id ? (<div className="flex-1 flex items-center gap-1">
                <Input value={editTitle} onChange={(e) => setEditTitle(e.target.value)} className="h-7 py-1 text-sm" autoFocus onClick={(e) => e.stopPropagation()} onKeyDown={(e) => {
                    if (e.key === "Enter")
                        saveEdit(chat.id);
                    if (e.key === "Escape")
                        cancelEdit();
